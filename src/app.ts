...
app.post("/partners/register", async (req, res) => {
  ...
  res
  .status(201)
  .json({id: partnersResult.insertId,name, user_id: userId, company_name, created_at: createdAt});
...
app.post("/customers/register", async (req, res) => {
  ...
  const userId = userResult.insertId;
  const [partnersResult] = await connection.execute<mysql.ResultSetHeader>('INSERT INTO customers (user_id, address, phone, created_at) VALUES(?,?,?,?)',
